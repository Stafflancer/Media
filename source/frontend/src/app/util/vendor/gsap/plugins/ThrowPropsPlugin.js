...
(_gsScope._gsQueue || (_gsScope._gsQueue = [])).push(function() {
  ...
  _gsScope._gsDefine(
    ...
    function(TweenPlugin, TweenLite, Ease, VelocityTracker) {
              ...
              if (
                ...
                if (typeof val === 'object') {
                  if (val.velocity !== undefined && typeof val.velocity === 'number') {
                    ...
                    velocities[p] =
                      tracker && tracker.isTrackingProp(p) ? tracker.getVelocity(p) : 0; //rotational values are actually converted to radians in CSSPlugin, but our tracking velocity is in radians already, so make it into degrees to avoid a funky conversion
                  }
